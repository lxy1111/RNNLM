...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			onehotx = make_onehot(x[t], self.vocab_size)

			net_in = self.V.dot(onehotx) + self.U.dot(s[t-1])
			s[t] = sigmoid(net_in)
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			onehotx = make_onehot(x[t], self.vocab_size)
			onehotd = make_onehot(d[t],self.vocab_size)

			sigma_out = (onehotd - y[t])*g

			self.deltaW += np.outer(sigma_out,s[t])

			sigma_in = self.W.T.dot(sigma_out) * f
			self.deltaV += np.outer(sigma_in, onehotx)
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			delta_out = (onehotd - y[t])*g

			self.deltaW += np.outer(delta_out,s[t])

			delta_in = self.W.T.dot(delta_out) * f
			self.deltaV += np.outer(delta_in, onehotx)
			...
			for tau in range(1,min(t,steps)+1):
				f = s[t-tau] * (1 - s[t-tau])
				delta_in = self.U.T.dot(delta_in)
				delta_in = delta_in * f

				onehotx = make_onehot(x[t-tau],self.vocab_size)
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		def _train(hdim,lookback, lr):
			...
			run_loss = r.train(X_train,D_train,X_dev,D_dev, epochs = 40, back_steps=lookback,learning_rate=lr)
	...
	if mode == "train-np":
		...
		run_loss = r.train_np(X_train, D_train, X_dev, D_dev,
							  back_steps=lookback, learning_rate=lr, epochs=40)
